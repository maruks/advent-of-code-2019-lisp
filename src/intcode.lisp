...
(defun do-step (program read-fn write-fn)
  (multiple-value-bind (opcode param-modes) (decode (aref (program-code program) (program-ip program)))
    (let ((next-ip (ecase opcode
		     (#+ccl 1 #-ccl #.+add+ (binary-op #'+ program param-modes))
		     (#+ccl 2 #-ccl #.+multiply+ (binary-op #'* program param-modes))
		     (#+ccl 3 #-ccl #.+in+ (read-input program read-fn param-modes))
		     (#+ccl 4 #-ccl #.+out+ (write-to-output program write-fn param-modes))
		     (#+ccl 5 #-ccl #.+jmp-if-not-zero+ (cond-jmp program (complement #'zerop) param-modes))
		     (#+ccl 6 #-ccl #.+jmp-if-zero+ (cond-jmp program #'zerop param-modes))
		     (#+ccl 7 #-ccl #.+less-than+ (compare program #'< param-modes))
		     (#+ccl 8 #-ccl #.+equals+ (compare program #'eql param-modes))
		     (#+ccl 9 #-ccl #.+adjust-base+ (adjust-base program param-modes))
		     (#+ccl 99 #-ccl #.+halt+ nil))))
