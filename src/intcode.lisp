...
(defun do-step (program read-fn write-fn)
  (multiple-value-bind (opcode param-modes) (decode (aref (program-code program) (program-ip program)))
    (let ((next-ip (ecase opcode
		     (#.+add+ (binary-op #'+ program param-modes))
		     (#.+multiply+ (binary-op #'* program param-modes))
		     (#.+in+ (read-input program read-fn param-modes))
		     (#.+out+ (write-to-output program write-fn param-modes))
		     (#.+jmp-if-not-zero+ (cond-jmp program (complement #'zerop) param-modes))
		     (#.+jmp-if-zero+ (cond-jmp program #'zerop param-modes))
		     (#.+less-than+ (compare program #'< param-modes))
		     (#.+equals+ (compare program #'eql param-modes))
		     (#.+adjust-base+ (adjust-base program param-modes))
		     (#.+halt+ nil))))
